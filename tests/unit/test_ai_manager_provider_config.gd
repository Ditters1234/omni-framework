...
func test_nobodywho_model_path_normalization_preserves_supported_path_forms() -> void:
	...
	assert_true(provider._is_remote_model_path("hf://owner/repo/model.gguf"))
	assert_true(provider._is_remote_model_path("https://example.com/model.gguf"))
	assert_true(provider._is_remote_model_path("huggingface:owner/repo/model.gguf"))
	assert_false(provider._is_remote_model_path("res://models/local.gguf"))
	provider.free()
