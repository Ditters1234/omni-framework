...
var _backend: RefCounted = EVENT_LOG_BACKEND.new()
...
func _refresh_state() -> void:
	...
	_render_rows(_read_dictionary_array(view_model.get("rows", [])), str(view_model.get("empty_label", "No events have been recorded.")))
...
func _build_row_text(row: Dictionary) -> String:
	...
	if not timestamp.is_empty():
		lines.append("%s • %s/%s" % [timestamp, str(row.get("domain", "")), str(row.get("signal_name", ""))])
	else:
		lines.append("%s/%s" % [str(row.get("domain", "")), str(row.get("signal_name", ""))])
