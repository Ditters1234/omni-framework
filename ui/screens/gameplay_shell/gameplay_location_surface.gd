...
func _render_location_actions() -> void:
	...
	if screens is Array and not screens.is_empty():
		for screen_entry in screens:
			...
			if _is_global_shell_surface(entry):
				continue
			if not LOCATION_PRESENCE_SERVICE.is_backend_entry_available(entry):
				continue
	...
	var entity_entries := LOCATION_PRESENCE_SERVICE.collect(_location_id, _location_template)
	for entity_entry in entity_entries:
		...
		for interaction in interactions:
			if not LOCATION_PRESENCE_SERVICE.is_backend_entry_available(interaction, entity_id):
				continue
