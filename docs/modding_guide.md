...
- When `auto_dispatch_first_reach_location` is `true`, the backend finds the selected quest's first `reach_location` objective for the assignee and starts `assignment_task_template_id` as a low-level travel task for that entity. The task moves the entity; the quest remains the objective and reward layer.
