...
Task visibility is also data-driven. `TaskActivitySummary` reads runtime task instances and their referenced task templates to build active/queued activity text for UI surfaces. `OwnedEntitiesBackend` and gameplay location presence rows use that shared summary to show what an entity is doing, where target-based work is headed, and how many ticks remain. The formatter uses task `display_name`, `target`, `remaining_ticks`, and `status`; it does not assume genre-specific job names.
