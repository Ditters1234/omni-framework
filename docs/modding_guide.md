...
`LocationPresenceService` resolves which NPCs appear at a location from three sources, merged and deduplicated. The gameplay location surface renders those rows without owning the merge logic:
...
Task visibility is also data-driven. `TaskActivitySummary` reads runtime task instances and their referenced task templates to build active/queued activity text for UI surfaces. `OwnedEntitiesBackend` and gameplay location presence rows use that shared summary to show what an entity is doing, where target-based work is headed, and how many ticks remain. The formatter uses task `display_name`, `target`, `remaining_ticks`, and `status`; it does not assume genre-specific job names. Owned-entity detail views can reorder queued task instances and cancel active or queued task instances directly; these controls operate on the generic runtime queue, so mods do not need special UI code for robots, companions, vehicles, crews, or any other owned entity type.
...
- `apply_condition`, `tick_condition`, and `expire_condition` are optional `ConditionEvaluator` dictionaries. The affected entity is available as `context:entity`; the runtime status-effect instance is available as `status_effect`; and the source template is available as `status_effect_template`. Failed `apply_condition` prevents the effect from being added, failed `tick_condition` skips that tick's actions, and failed `expire_condition` skips `on_expire` actions while still removing the expired runtime instance.
...
`CraftingBackend` requires `station_id`; optional `recipe_tags` and `recipe_ids` filter visible recipes. The backend rechecks those filters, station requirements, and discovery state when craft is confirmed, so a recipe that is not visible from the current station cannot be crafted by stale UI state. Instant recipes produce output immediately. Recipes with `craft_time_ticks > 0` consume inputs immediately and start the configured timed craft task, which grants the output when completed. Resolve order is recipe `craft_task_template_id`, backend `craft_task_template_id`, then `crafting.default_task_template_id` from config. The base mod points that config key at its own `CRAFT` task shell; total conversions should provide their own equivalent task template or leave the key empty to disable timed crafting.
...
- `stat_check` — stat comparison. Fields: `stat`, `op` (`>=`, `>`, `<=`, `<`, `==`, `!=`), `value`, optional `entity_id`. Use `value_stat` instead of `value` to compare against another stat on the same resolved entity, or provide `value_entity_id` to compare against another context entity.
...
- When `auto_dispatch_first_reach_location` is `true`, the backend starts the quest with assignment dispatch metadata. `QuestTracker` finds each active stage's first `reach_location` objective for the assignee and starts `assignment_task_template_id` as a low-level travel task for that entity. Multi-stage quests continue dispatching as each assignee reach-location stage advances. The task moves the entity; the quest remains the objective and reward layer.
...
The current entity sheet exposes stats, active status effects, currency balances, equipped parts, inventory summaries, faction standing, data-authored inventory use actions, direct equip for loose inventory parts, discard actions for loose inventory, per-instance favorite/lock flags, progress/activity rows, and handoff into equipment management. Status effect rows are derived from `GameState.active_status_effects` and the referenced `status_effects.json` templates, including display names, descriptions, remaining duration, stacks, tags, and data-authored stat modifiers. Direct equip is still data-driven: the backend derives compatible and recommended slots from the target entity's `provides_sockets`, part `tags`, and `required_tags`; no slot ids or item categories are hardcoded by the UI. The screen owns rendering and input wiring only; `EntitySheetBackend` owns inventory use, equip, discard, favorite/lock mutations, and progress/activity view-model assembly.
...
- `cancel_label`
- `empty_label`
- `assignment_task_template_id` - optional task template used for reach-location assignment dispatch. If omitted, the backend uses `tasks.default_assignment_task_template_id` or `tasks.default_travel_task_template_id` from config.
