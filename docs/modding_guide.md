...
*   `"push_screen"`: Push a routed UI screen. Requires `screen_id`; optional `params` dictionary. `screen_id` must match a known engine route or backend route id from the shared UI route catalog.
