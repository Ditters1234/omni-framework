...
- **20 backend-driven screens:** `AssemblyEditorBackend`, `ExchangeBackend`, `ListBackend`, `ChallengeBackend`, `TaskProviderBackend`, `ActivityBoardBackend`, `ScheduleBackend`, `CatalogListBackend`, `CraftingBackend`, `DialogueBackend`, `EncounterBackend`, `EntitySheetBackend`, `OwnedEntitiesBackend`, `LootBackend`, `RewardReviewBackend`, `ActiveQuestLogBackend`, `FactionReputationBackend`, `AchievementListBackend`, `EventLogBackend`, `WorldMapBackend`
...
- `ActivityBoardBackend` powers player-facing activity selection. It filters data-authored activities, shows available/locked/upcoming state from `ActivityService` and `ActivityScheduleService`, displays category/location/provider metadata and cached AI flavor, and executes selected activities through `ActivityService`.
- `ScheduleBackend` powers read-only activity schedule projection. It renders day, range, configured-week, and configured-month views from `ActivityScheduleService`, groups slots by day/category/location/status, and leaves activity execution to activity board/service paths.
