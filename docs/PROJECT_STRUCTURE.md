...
- `EntitySheetBackend` powers the character menu, including stats, active status effects, equipment, quests, reputation, progress, activity, a searchable/filterable inventory browser, data-authored usable inventory items, direct equip from data-derived socket/tag compatibility, discard actions for loose inventory, per-instance favorite/lock controls, and equipment handoff.
