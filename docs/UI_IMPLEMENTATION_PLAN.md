...
- **Backend helper strategy:** `backend_helpers.gd` contains phase-neutral view-model/data helpers shared by multiple backends, while `backend_navigation_helper.gd` owns backend-screen routing, gameplay-shell surface navigation, and JSON action dispatch. Assembly-editor-only logic stays in `assembly_editor_config.gd` and `assembly_editor_option_provider.gd` so generic helpers do not become a junk drawer.
...
Current status: interaction pass complete. `WorldMapBackend` builds a graph view model from `LocationGraph.get_all_locations()`, resolves faction tint from explicit `faction_id` or faction territory, supports optional authored `map_position` coordinates, and falls back to a deterministic circular layout when `map_position` is omitted or empty. The routed `world_map` screen renders node buttons, route lines, travel costs, and travels on node click using the cheapest routed total travel cost between the current location and the selected destination. The graph supports mouse-wheel zoom, drag panning, fit/current centering controls, and radial/horizontal/vertical orientation modes for different map shapes. At lower zoom levels it now declutters automatically by shrinking nodes into compact pills or markers and suppressing edge travel-cost labels until they are legible again. Initial fit/center logic uses the graph's effective viewport size and schedules a deferred post-layout refresh so nodes stay visible even when the screen renders before Godot assigns final layout size inside hosted UI containers. Fog-of-war and region overlays remain deferred additions.
