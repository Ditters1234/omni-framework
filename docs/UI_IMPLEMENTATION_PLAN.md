...
Current status: interaction pass complete. `WorldMapBackend` builds a graph view model from `LocationGraph.get_all_locations()`, resolves faction tint from explicit `faction_id` or faction territory, supports optional authored `map_position` coordinates, and falls back to a deterministic circular layout when `map_position` is omitted or empty. The routed `world_map` screen renders node buttons, route lines, travel costs, and travels on node click using the cheapest routed total travel cost between the current location and the selected destination. The graph supports mouse-wheel zoom, drag panning, fit/current centering controls, and radial/horizontal/vertical orientation modes for different map shapes. Initial fit/center logic uses the graph's effective viewport size and schedules a deferred post-layout refresh so nodes stay visible even when the screen renders before Godot assigns final layout size inside hosted UI containers. Fog-of-war and region overlays remain deferred additions.
