...
- **`encounters.json` data file, `EncounterRegistry`, `EncounterBackend`, `encounter_screen.tscn`.** V1 now exists. Deferred pieces are tag-duration effects, custom log/feed components, advanced strategies, and persisted `EncounterInstance` save/load.
...
| File | Purpose |
|---|---|
| `mods/base/data/encounters.json` | Base game encounter templates. Currently ships `base:tutorial_brawl`. |
...
| `ui/components/encounter_action_button.gd` + `.tscn` | Deferred. V1 uses stock `Button` nodes generated by `encounter_screen.gd`. |
| `ui/components/encounter_log_feed.gd` + `.tscn` | Deferred. V1 renders the capped log feed directly in `encounter_screen.gd`. |
...
| `tests/unit/test_encounter_runtime.gd` | Deferred helper-specific suite. Runtime behavior is covered through backend tests in v1. |
| `tests/integration/test_encounter_flow.gd` | Deferred end-to-end interaction suite. |
...
- Created `mods/base/data/encounters.json` with `base:tutorial_brawl`.
...
- Unit tests: backend coverage exists; registry-specific validation and patch round-trip tests remain follow-up coverage.
...
- Author remaining reference encounters (`base:tutorial_negotiation`, `base:tutorial_endurance`) in `mods/base/data/encounters.json`.
...
**Acceptance:** Reference encounters are playable from gameplay. Modders have enough docs to author another without engine changes.
