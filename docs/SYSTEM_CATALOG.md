...
|---|---|---|---|---|
| `ActionDispatcher` | `ActionDispatcher` | `systems/action_dispatcher.gd` | DataManager | Executes JSON action blocks from quests/tasks: `give_currency`, `travel`, `spawn_entity`, `start_quest`, `apply_status_effect`, `learn_recipe`, `modify_reputation`, time advancement, event recording, etc. See [`modding_guide.md`](modding_guide.md) for schema. |
...
| `GameplayLocationActionService` | `GameplayLocationActionService` | `ui/screens/gameplay_shell/gameplay_location_action_service.gd` | UIRouter, GameState, LocationAccessService | Shell-scoped boundary for location-surface route validation, shell surface opens, and travel mutation. |
...
| `TaskProviderBackend` | `ui/screens/backends/task_provider_screen.tscn` | Faction ID, contract list | Faction contract board that starts quest templates from `faction.quest_pool`, with optional cached AI flavor text and owned-entity auto-dispatch through assignee reach-location objectives. | [`modding_guide.md`](modding_guide.md) |
| `ActivityBoardBackend` | `ui/screens/backends/activity_board_screen.tscn` | Activity filters and display flags | Activity board that renders available, locked, and upcoming activity rows, shows schedule/location/provider metadata and cached AI flavor, and executes the selected activity through `ActivityService`. | [`modding_guide.md`](modding_guide.md) |
| `ScheduleBackend` | `ui/screens/backends/schedule_screen.tscn` | Activity filters, view mode, grouping, and past-slot display | Read-only activity schedule projection for day, range, configured-week, and configured-month views, with slot statuses from `ActivityService` and `ActivityScheduleService`. | [`modding_guide.md`](modding_guide.md) |
